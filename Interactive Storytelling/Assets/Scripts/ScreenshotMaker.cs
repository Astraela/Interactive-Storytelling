...
/*
        ...
        for (float i = bounds.w, xPos = 0; i < bounds.x; i += renderCamera.aspect * renderCamera.orthographicSize * 2, xPos++)
        {
            for (float j = bounds.y, yPos = 0; j < bounds.z; j += renderCamera.aspect * renderCamera.orthographicSize * 2, yPos++)
            {
                ...
                if(yPos == 1)
                virtualPhoto.ReadPixels(new Rect(0, resolution*.6675f, resolution, resolution), (int)xPos * resolution, (int)yPos * resolution);
                else
        ...
        print(path+"PortaltoAnotherDreamScreenshot.png");

        System.IO.File.WriteAllBytes(path+"\\PortaltoAnotherDreamScreenshot" + DateTime.Now.ToString("yyyy-MM-dd HH.mm.ss") + ".png", bytes);
